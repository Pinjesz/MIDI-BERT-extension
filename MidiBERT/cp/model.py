...
class MidiBert(nn.Module):
    def __init__(self, bertConfig, e2w, w2e):
        ...
        self.bar_pad_word = self.e2w['Bar']['Pad_None']
        self.mask_word_np = np.array([self.e2w[etype]['Mask_None'] for etype in self.e2w], dtype=np.longlong)
        self.pad_word_np = np.array([self.e2w[etype]['Pad_None'] for etype in self.e2w], dtype=np.longlong)
