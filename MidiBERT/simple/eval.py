...
def get_args():
    ...
    root = f"result/{args.tokenization}/finetune/"
...
def conf_mat(_y, output, task, tokenization, name):
    if task == "melody":
        target_names = ["M", "B", "A"]
        seq = False
    elif task == "velocity":
    ...
    _title = f"BERT ({tokenization}): " + task + " task"

    save_cm_fig(cm, classes=target_names, normalize=True, title=_title, seq=seq, path=f'result/{tokenization}/eval/{name}/cm_{tokenization}_{task}.jpg')
...
def main():
    ...
    if args.use_wandb:
        ...
        if args.old:
            wandb.init(
                ...
                job_type=f'eval_{args.task}_{args.tokenization}',
                config=config
            )
        else:
    ...
    conf_mat(y_test, all_output, args.task, args.tokenization, args.name)
