...
class BERTTrainer:
    ...
    def iteration(self, training_data, max_seq_len, train=True):
        ...
        for ori_seq_batch in pbar:
            ...
            ori_seq_batch = ori_seq_batch.to(torch.long).to(self.device)
            ...
            attn_mask = (
                (ori_seq_batch != self.midibert.pad_word).float().to(self.device)
            )  # (4,512)
        ...
        return round(total_loss / len(training_data), 4), round(
            total_acc / len(training_data), 4
        )
