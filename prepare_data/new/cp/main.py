...
from dict.make_dict import make_cp_new, add_to_cp
from config import *
...
def get_args():
    ...
    ### path ###
    parser.add_argument("--dict", type=str, default="dict/cp_new.pkl")
    parser.add_argument(
        "--dataset",
        type=str,
    ...
    ### output ###
    parser.add_argument("--output_dir", default="Data/cp_new")
    parser.add_argument(
        "--name", default=""
...
def padding(data, max_len, e2w, ans=False):
    ...
    for _ in range(pad_len):
        if not ans:
            data.append([e2w[etype]["Pad_None"] for etype in e2w])
        else:
            data.append(0)
    return data


def post_process(events: list[list[miditok.Event]], e2w, task):
    ids: list[list[int]] = []
    ys: list[int] = []
    ...
    for i in range(len(events)):
        ...
        if e[1].type == "Bar":
            bar_new = True
        elif e[1].type == "Position":
            position = str(e[1])
        else:
            bar = "Bar_New" if bar_new else "Bar_Continue"
            pitch = str(e[2])
            duration = str(e[4])
            velocity = (
                np.searchsorted(DEFAULT_VELOCITY_BINS, e[3].value, side="right") - 1
            )
            melody = e[5].value
            ...
            ids.append(
                [
                    e2w["Bar"][bar],
                    e2w["Position"][position],
                    e2w["Pitch"][pitch],
                    e2w["Duration"][duration],
                ]
            )
...
def extract(files: list[str], args, tokenizer: miditok.MIDITokenizer, e2w, mode=""):
    ...
    for path in tqdm(files):
        ...
        while not success:
            try:
                ...
                tokenizer, e2w = add_to_cp(tokenizer, e2w, e.args[0])
    ...
    if args.task != "":
        if segments.shape[0] != ans.shape[0]:
            print(
                f"Answer shape: {ans.shape} does not fit data shape: {segments.shape}"
            )
            exit(1)
...
def main():
    ...
    tokenizer, vocabulary = make_cp_new(config)
