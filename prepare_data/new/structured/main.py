...
def get_args():
    ...
    ### output ###
    parser.add_argument("--output_dir", default="Data/structured_new")
    parser.add_argument(
        "--name", default=""
    ...
    for i in range(len(events)):
        ...
        if e.type == "Program":
            if events[i + 1].type == "Pitch" and events[i + 2].type == "Velocity":
                ...
                result.append(Event(events[i + 1], e.value, vel))
...
def main():
    ...
    tokenizer, vocabulary = make_simple_new(config, "structured")
